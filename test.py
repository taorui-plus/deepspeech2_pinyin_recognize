...
def test(model, test_fn, datagen, mb_size=100, conv_context=11,
    ...
    for batch in datagen.iterate_test(mb_size):
        ...
        for i, prediction in enumerate(predictions):
            # print (ground_truth[i], argmax_decode(prediction))
            sm = edit_distance.SequenceMatcher(a=ground_truth[i],b=argmax_decode(prediction))
            Truth = ground_truth[i]
            Prediction = argmax_decode(prediction)
            
            #with open('pridiction.json') as f:
            #    f.write(json.dumps('prediction: {}'.format(Prediction)))
            #    f.write("\n")

            Distance = str(sm.distance()/len(ground_truth[i])) 
            print('{"Truth": "%s", "Prediction": "%s", "Ditance": "%s"}'%(Truth, Prediction, Distance))
        avg_cost += ctc_cost
        i += 1
    return avg_cost / i
